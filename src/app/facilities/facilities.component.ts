...
export class FacilitiesComponent implements OnInit {
  ...
  getVehicleImage(vehicleTypeId: string): string {
    if (!vehicleTypeId) return '../../../../assets/images/icons/helmet.svg';
    switch (vehicleTypeId) {
      ...
      default:
        return '../../../../assets/images/icons/helmet.svg';
    }
  }
  ...
  fetchVehicleData() {
    ...
    this.userService.getParkingFacility(parkingId, page, orderBy, limit, offset, filters).subscribe({
      next: (response: ApiResponse) => {
        if (response && Array.isArray(response.rows)) {
          const mappedData = response.rows.map((item: any) => {

            const vehicle = item.parkingTransaction?.vehicle || {};
  ...
  private getVehicleIcon(vehicleTypeId: string | undefined): string {
    ...
    switch (vehicleTypeId) {
      ...
      default: return '../../../../assets/images/icons/helmet.svg';
    }
  }

  private getVehicleBgColor(vehicleTypeId: string | undefined): string {
    if (!vehicleTypeId) return '#000';
    switch (vehicleTypeId) {
      case 'e7e2ffc1-faec-48b5-b8b1-735f28c1d3ab': return '#000';
      case '3d3f97d3-4cf0-45a6-9bbd-15ab5a6df8d6': return '#000';
      default: return '#000';
    }
  }
