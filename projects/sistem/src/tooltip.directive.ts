...
export class TooltipDirective {
    ...
    hideAllTooltips() {
        ...
        tooltips.forEach(tooltip => {
            this.renderer.removeClass(tooltip, 'ui-tooltip-show'); // Use Renderer2
            tooltip.remove();
        });
        this.tooltip = undefined; // Reset current tooltip
    }

    create() {
        this.tooltip = document.createElement('span');
        this.tooltip.classList.add('ui-tooltip', this.placement);
        this.tooltip.textContent = this.tooltipTitle;
        document.body.appendChild(this.tooltip);
    }
